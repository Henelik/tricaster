...
func TestShading(t *testing.T) {
	...
	col := DefaultWorld.Shade(i.ToHit(r), 0, []ray.Intersection{*i})
	...
	col2 := w.Shade(i2.ToHit(r2), 0, []ray.Intersection{*i2})
...
func TestWorldRefractRefract(t *testing.T) {
	...
	a.Mat = &shading.PhongMat{
		...
		Transparency: 0,
		IOR:          1,
		Pattern:      shading.NewTestPattern(nil),
	}
	b := w.Geometry[1].(*geometry.Sphere)
	b.Mat = &shading.PhongMat{
		Ambient:      0,
		Diffuse:      0,
		Specular:     0,
		Shininess:    0,
