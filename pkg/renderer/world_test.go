...
func TestIntersect(t *testing.T) {
	...
	got := ray2.SortI(DefaultWorld.Intersect(
		ray2.NewRay(
...
func TestShading(t *testing.T) {
	...
	r := ray2.NewRay(tuple.NewPoint(-5, 0, 0), tuple.Right)
	s := DefaultWorld.Geometry[0]
	i := &ray2.Intersection{4, s}
	col := DefaultWorld.Shade(i.ToHit(r, []ray2.Intersection{*i}), 0)
	...
	r2 := ray2.NewRay(tuple.Origin, tuple.Right)
	s2 := DefaultWorld.Geometry[1]
	i2 := &ray2.Intersection{0.5, s2}
	col2 := w.Shade(i2.ToHit(r2, []ray2.Intersection{*i2}), 0)
	assert.Equal(t, color.Grey(0.9049844720832575), col2)
}

func TestColorAtMiss(t *testing.T) {
	// The color when a ray misses
	r := ray2.NewRay(tuple.NewPoint(0, 0, -5), tuple.Forward)
...
func TestShadow(t *testing.T) {
	w := *DefaultWorld
	w.Config = &WorldConfig{Shadows: true, MaxBounce: 1}
	testCases := []struct {
		name string
		p    *tuple.Tuple
		want bool
...
func TestWorldRefractOpaque(t *testing.T) {
	s := DefaultWorld.Geometry[0]
	r := ray2.NewRay(tuple.NewPoint(0, 0, -5), tuple.NewVector(0, 0, 1))
	inters := []ray2.Intersection{{4, s}, {6, s}}
...
func TestWorldRefractTotalInternalReflection(t *testing.T) {
	...
	r := ray2.NewRay(tuple.NewPoint(0, 0, math.Sqrt2/2), tuple.NewVector(0, 1, 0))
	inters := []ray2.Intersection{{-math.Sqrt2 / 2, s}, {math.Sqrt2 / 2, s}}
...
func TestWorldRefractRefract(t *testing.T) {
	...
	r := ray2.NewRay(tuple.NewPoint(0, 0, 0.1), tuple.NewVector(0, 1, 0))
	inters := []ray2.Intersection{{-0.9899, a}, {-0.4899, b}, {0.4899, b}, {0.9899, a}}
